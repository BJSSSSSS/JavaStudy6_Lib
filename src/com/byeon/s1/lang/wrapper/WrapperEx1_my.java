...
public class WrapperEx1_my {
		...
		while(flag) {
			...
			if(zumin.length() == 14 && checkBox <=1) {
				...
				for(int i=0; i<result1.length(); i++) {
					...
					//위에서 "" + 하는 이유가 문자열 + char타입하면 
					//문자열이 되기때문에 더해주는것임 기억해라!
					//문자열을 만들고 거기서 integer를해서 int타입으로 만든뒤 num을 곱한것임
					//이걸 count에 더해주고 num은 ++ 해서 다음 i일때 num은 1이 더해진상태
				}
	...
	public void ex1() {
		...
		// 쉬운방법은 %2 해서 나머지 1 or 0 으로 결정, 대신 외국인은 고려 어렵다
...
//		int myBirth = Integer.parseInt(zuminBirth);
		...
		// 쉬운방법은 else를 다 겨울로 묶는것 사실 이 외계인은 아니다 쓸수는 있는 변수긴 하다
